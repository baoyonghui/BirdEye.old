...
class FlyLineRenderer : MKOverlayRenderer {
    // Static properties
    ...
    var liveAircraftPoint: CGPoint? {
        let polygon = self.overlay as? FlyLinePolygon
        var point: CGPoint?
    ...
    override func draw(_ mapRect: MKMapRect, zoomScale: MKZoomScale, in context: CGContext) {
        ...
        let polygon = self.overlay as? FlyLinePolygon
        if polygon != nil && polygon!.flyLineState != nil {
            self.context = context
            self.zoomScale = zoomScale
            computeGeometries(from: polygon!.coordinates)
            if canDraw {
                let state = polygon!.flyLineState!
                ...
                drawHull()
                drawMeander()
                drawAircraftLine(state == .managing
                                 ? lastAircraftPoint
                                 : liveAircraftPoint)
            }
        }
...
extension FlyLineRenderer {
    private func computeGeometries(from coordinates: [CLLocationCoordinate2D]) {
        ...
        for coordinate in coordinates {
            let p:MKMapPoint = MKMapPoint(coordinate)
            points.append(point(for: p))
        }
...
extension FlyLineRenderer {
    ...
    private func drawVertices() {
        for point in pointSet.points {
            ...
            (self.overlay as? FlyLinePolygon)?.vertexRadius = Double(meterRadius)
            let path = CGMutablePath()
